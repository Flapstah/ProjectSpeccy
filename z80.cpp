...
void CZ80::OutputStatus(void) const
{
	...
	fprintf(stdout, "[Z80]        BC'=%04X  BC=%04X  B=%02X  C=%02X  (%02X %02X %02X %02X %02X %02X %02X %02X)\n", m_BCalt, m_BC, m_B, m_C, ReadMemory(m_BC), ReadMemory(m_BC + 1), ReadMemory(m_BC + 2), ReadMemory(m_BC + 3), ReadMemory(m_BC + 4), ReadMemory(m_BC + 5), ReadMemory(m_BC + 6), ReadMemory(m_BC + 7));
	fprintf(stdout, "[Z80]        DE'=%04X  DE=%04X  D=%02X  E=%02X  (%02X %02X %02X %02X %02X %02X %02X %02X)\n", m_DEalt, m_DE, m_D, m_E, ReadMemory(m_DE), ReadMemory(m_DE + 1), ReadMemory(m_DE + 2), ReadMemory(m_DE + 3), ReadMemory(m_DE + 4), ReadMemory(m_DE + 5), ReadMemory(m_DE + 6), ReadMemory(m_DE + 7));
	fprintf(stdout, "[Z80]        HL'=%04X  HL=%04X  H=%02X  L=%02X  (%02X %02X %02X %02X %02X %02X %02X %02X)\n", m_HLalt, m_HL, m_H, m_L, ReadMemory(m_HL), ReadMemory(m_HL + 1), ReadMemory(m_HL + 2), ReadMemory(m_HL + 3), ReadMemory(m_HL + 4), ReadMemory(m_HL + 5), ReadMemory(m_HL + 6), ReadMemory(m_HL + 7));
	fprintf(stdout, "[Z80]        IX=%04X  (%02X %02X %02X %02X %02X %02X %02X *%02X* %02X %02X %02X %02X %02X %02X %02X)\n", m_IX, ReadMemory(m_IX - 7), ReadMemory(m_IX - 6), ReadMemory(m_IX - 5), ReadMemory(m_IX - 4), ReadMemory(m_IX - 3), ReadMemory(m_IX - 2), ReadMemory(m_IX - 1), ReadMemory(m_IX), ReadMemory(m_IX + 1), ReadMemory(m_IX + 2), ReadMemory(m_IX + 3), ReadMemory(m_IX + 4), ReadMemory(m_IX + 5), ReadMemory(m_IX + 6), ReadMemory(m_IX + 7));
	fprintf(stdout, "[Z80]        IY=%04X  (%02X %02X %02X %02X %02X %02X %02X *%02X* %02X %02X %02X %02X %02X %02X %02X)\n", m_IY, ReadMemory(m_IY - 7), ReadMemory(m_IY - 6), ReadMemory(m_IY - 5), ReadMemory(m_IY - 4), ReadMemory(m_IY - 3), ReadMemory(m_IY - 2), ReadMemory(m_IY - 1), ReadMemory(m_IY), ReadMemory(m_IY + 1), ReadMemory(m_IY + 2), ReadMemory(m_IY + 3), ReadMemory(m_IY + 4), ReadMemory(m_IY + 5), ReadMemory(m_IY + 6), ReadMemory(m_IY + 7));
	fprintf(stdout, "[Z80]        I=%02X  R=%02X  IM=%i  IFF1=%i  IFF2=%i\n", m_I, m_R, m_State.m_InterruptMode, m_State.m_IFF1, m_State.m_IFF2);
	fprintf(stdout, "[Z80]        SP=%04X (%02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X)\n", m_SP, ReadMemory(m_SP), ReadMemory(m_SP + 1), ReadMemory(m_SP + 2), ReadMemory(m_SP + 3), ReadMemory(m_SP + 4), ReadMemory(m_SP + 5), ReadMemory(m_SP + 6), ReadMemory(m_SP + 7), ReadMemory(m_SP + 8), ReadMemory(m_SP + 9), ReadMemory(m_SP + 10), ReadMemory(m_SP + 11), ReadMemory(m_SP + 12), ReadMemory(m_SP + 13), ReadMemory(m_SP + 14), ReadMemory(m_SP + 15));
		...
		switch (ReadMemory(address))
		{
			...
			case 0xDD: // Prefix DD
				switch (ReadMemory(address + 1))
				{
					...
					case 0x2E: // LD IXl,n
						return DecodeLDIXln(address, pMnemonic);
						break;
			...
			case 0xFD: // Prefix FD
				switch (ReadMemory(address + 1))
				{
					...
					case 0x2E: // LD IYl,n
						return DecodeLDIYln(address, pMnemonic);
						break;
...
uint32 CZ80::ImplementLDIXhn(void)
{
	//
	// Operation:	IXh <- n
	// Op Code:		LD
...
uint32 CZ80::ImplementLDr_IXd_(void)
{
	...
	++m_PC;
	REGISTER_8BIT(opcode >> 3) = ReadMemory(m_IX + displacement);
	return 19;
}
...
uint32 CZ80::ImplementLDr_IYd_(void)
{
	...
	int8 displacement = static_cast<int8>(ReadMemory(++m_PC));
	++m_PC;
	REGISTER_8BIT(opcode >> 3) = ReadMemory(m_IY + displacement);
	return 19;
}
...
uint32 CZ80::ImplementLDHL_nn_(void)
{
	...
	m_addresshi = ReadMemory(++m_PC);
	++m_PC;
	m_L = ReadMemory(m_address++);
	m_H = ReadMemory(m_address);
	return 16;
}
...
uint32 CZ80::ImplementLDdd_nn_(void)
{
	...
	m_addresshi = ReadMemory(++m_PC);
	++m_PC;
	REGISTER_16BIT_LO(opcode >> 4) = ReadMemory(m_address++);
	REGISTER_16BIT_HI(opcode >> 4) = ReadMemory(m_address);
	return 20;
}
...
uint32 CZ80::ImplementLDIX_nn_(void)
{
	...
	m_addresslo = ReadMemory(m_PC++);
	m_addresshi = ReadMemory(m_PC++);
	m_IXl = ReadMemory(m_address++);
	m_IXh = ReadMemory(m_address);
	return 20;
}
...
uint32 CZ80::ImplementLDIY_nn_(void)
{
	...
	m_addresslo = ReadMemory(m_PC++);
	m_addresshi = ReadMemory(m_PC++);
	m_IYl = ReadMemory(m_address++);
	m_IYh = ReadMemory(m_address);
	return 20;
}
...
uint32 CZ80::ImplementINr_C_(void)
{
	...
	m_addresslo = m_C;
	m_addresshi = m_B;
	REGISTER_8BIT(ReadMemory(m_PC++) >> 3) = ReadPort(m_address);
	return 12;
}
...
uint32 CZ80::ImplementOUT_C_r(void)
{
	...
	m_addresslo = m_C;
	m_addresshi = m_B;
	WritePort(m_address, REGISTER_8BIT(ReadMemory(m_PC++) >> 3));
	return 12;
}
...
void CZ80::DecodeLDIXhn(uint16& address, char* pMnemonic) const
{
	sprintf(pMnemonic, "LD IXh,%02X", ReadMemory(address + 2));
	address += 3;
}
...
void CZ80::DecodeLDIYhn(uint16& address, char* pMnemonic) const
{
	sprintf(pMnemonic, "LD IYh,%02X", ReadMemory(address + 2));
	address += 3;
}
